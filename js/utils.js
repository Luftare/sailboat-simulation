...
function handleBallsCollision(a, b) {//expect that balls a and b intersect
  ...
  pA.set(a.position).substract(b.position).toLength(depth / 2);
  pB.set(b.position).substract(a.position).toLength(depth / 2);
  a.position.add(pA);
  b.position.add(pB);
...
function pushCircleFromSegment(c, a, b, depth) {
  const ABn = (new Vector(b)).substract(a).normalize().rotate(Math.PI / 2);
...
function pointSegmentDistance(c, a, b) {//based on: https://stackoverflow.com/a/6853926
  let x = c.x;
  let y = c.y;
...
function getLinesIntersection(a, b, c, d) {//based on: https://stackoverflow.com/a/38977789
  let x1 = a.x;
  let y1 = a.y;
  let x2 = b.x;
  let y2 = b.y;
...
function getSegmentsIntersection(s1, e1, s2, e2) {//based on: http://jsfiddle.net/justin_c_rounds/Gd2S2/light/
