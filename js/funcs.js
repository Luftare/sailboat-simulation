...
const generateGates = islands => {
  ...
  islands.forEach(s => {
    if(s.polygon.pointInside(first) || s.polygon.pointInside(other)) isValid = false;
  });
  
  return isValid? [
    new Seamark(first, "red"),
    new Seamark(other, "green")
  ] : [];
};
...
const generatePath = (size, r) => {
	...
	for(let i = 0; i < count; i++) {
		...
		const dir = last.dir.clone().rotate((Math.random() - 0.5) * 1).normalize().scale(r * 4 * (Math.random() + 1));
		res.push({
...
const generateEnvironment = () => {
	...
	const arr = (new Array(9)).fill(1).map(s => ({
			pos: (new Vector()).random(Math.random() * game.zMap.canvas.width / 2 - maxR),
	...
	while(overlapping) {
		...
		arr.forEach(s => {
			if(s.dir) return;
			const closest = arr.filter(o => o !== s).sort((a, b) => (a.pos.distance(s.pos) - (a.r + s.r)) -  (b.pos.distance(s.pos) - (b.r + s.r)))[0];
			if(closest.pos.distance(s.pos) < closest.r + s.r) {
				overlap = true;
				const avoid = s.pos.clone().substract(closest.pos).normalize().scale(10);
				s.pos.add(avoid);
			}
		})
		overlapping = overlap;
	}
	const islands = arr.filter(s => s.pos.distance(center) > s.r).filter(s => !s.dir).map(s => new Island(s.pos, s.r));
	const seamarks = [];
	arr.filter(s => s.dir).forEach(s => {
		seamarks.push(new Seamark(s.dir.clone().rotate(Math.PI / 2).scale(gateR).add(s.pos), "green"));
		seamarks.push(new Seamark(s.dir.clone().rotate(-Math.PI / 2).scale(gateR).add(s.pos), "red"));
	});
