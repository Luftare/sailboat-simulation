class View {
  ...
  preDraw() {
    ...
    const now = Date.now();
    this.fx.shakeCounter = Math.max(
      0,
      this.fx.shakeCounter - this.fx.shakeDamp
    );
    ...
    this.ctx.translate(
      Math.cos(now * this.fx.shakeFrq) *
        this.fx.shakeAmplitude *
        this.fx.shakeCounter,
      Math.sin(now * this.fx.shakeFrq) *
        this.fx.shakeAmplitude *
        this.fx.shakeCounter
    );
    this.ctx.translate(
  ...
  drawImage(img, x, y, angle = 0, scaleX = 1, scaleY = 1, shift = 0) {
    ...
    ctx.rotate(angle);
    ctx.translate(
      -x - (scaleX * img.width) / 2,
      -y - (scaleY * img.height) / 2
    );
